...
from dataset import FeverStage1RobertaDataset, roberta_collate_fn
from roberta_relevance_model import RobertaRelevanceScorer

def train_roberta(roberta_config: RobertaConfig, device: torch.device, train_data: list[dict], test_data: list[dict]):
    ...
    optimizer = torch.optim.AdamW([
        {"params": model.roberta.encoder.layer[-roberta_config.stage1_unfreeze_layers:].parameters(), "lr": roberta_config.learning_rate},
    ...
    for epoch in range(roberta_config.stage1_epochs):
        model.train()
        total_loss = 0
        ...
        print(f"Epoch {epoch + 1}/{roberta_config.stage1_epochs} — Loss: {total_loss / len(train_loader):.4f}", flush=True)
